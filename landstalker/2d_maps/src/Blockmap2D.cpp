...
TilePoint Blockmap2D::XYToTilePoint(const Point& point) const
{
	return TilePoint({ (point.x - GetLeft()) / GetBlockWidth(), (point.y - GetTop()) / GetBlockHeight()});
}

Point Blockmap2D::ToXYPoint(const TilePoint& point) const
{
	return Point{static_cast<int>((point.x + GetLeft()) * GetBlockWidth()),
		static_cast<int>((point.y * GetBlockHeight()) + GetTop())};
}
