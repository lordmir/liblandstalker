...
bool GameData::Open(const std::filesystem::path& asm_file)
{
	...
	try
	{
		...
		SetProgress("Loading Graphics data from ASM...", 1.0 / 7.0);
		m_gd = std::make_shared<GraphicsData>(asm_file);
		m_data.push_back(m_gd);
		SetProgress("Loading String data from ASM...", 2.0 / 7.0);
		m_sd = std::make_shared<StringData>(asm_file);
		m_data.push_back(m_sd);
		SetProgress("Loading Sprite data from ASM...", 3.0 / 7.0);
		m_spd = std::make_shared<SpriteData>(asm_file);
		m_data.push_back(m_spd);
		SetProgress("Loading Script data from ASM...", 4.0 / 7.0);
		m_scd = std::make_shared<ScriptData>(asm_file);
		m_data.push_back(m_scd);
		SetProgress("Loading Audio data from ASM...", 5.0 / 7.0);
		m_ad = std::make_shared<AudioData>(asm_file);
		m_data.push_back(m_ad);
		SetProgress("Loading Music data from ASM...", 6.0 / 7.0);
		m_md = std::make_shared<MusicData>(asm_file);
		m_data.push_back(m_md);
		CacheData();
		SetDefaults();
...
bool GameData::Open(const Rom& rom)
{
	...
	try
	{
		...
		m_data.push_back(m_rd);
		SetProgress("Loading Graphics data from ROM...", 1.0 / 7.0);
		m_gd = std::make_shared<GraphicsData>(rom);
		m_data.push_back(m_gd);
		SetProgress("Loading String data from ROM...", 2.0 / 7.0);
		m_sd = std::make_shared<StringData>(rom);
		m_data.push_back(m_sd);
		SetProgress("Loading Sprite data from ROM...", 3.0 / 7.0);
		m_spd = std::make_shared<SpriteData>(rom);
		m_data.push_back(m_spd);
		SetProgress("Loading Script data from ROM...", 4.0 / 7.0);
		m_scd = std::make_shared<ScriptData>(rom);
		m_data.push_back(m_scd);
		SetProgress("Loading Audio data from ROM...", 5.0 / 7.0);
		m_ad = std::make_shared<AudioData>(rom);
		m_data.push_back(m_ad);
		SetProgress("Loading Music data from ROM...", 6.0 / 7.0);
		m_md = std::make_shared<MusicData>(rom);
		m_data.push_back(m_md);
		CacheData();
		SetDefaults();
		SetProgress("Done", 1.0);
