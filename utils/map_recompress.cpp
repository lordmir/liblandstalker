...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "-o" || arg == "--outdir") && i + 1 < argc) {
            ...
            if (fs::is_regular_file(p)) {
                ...
                std::cerr << "Warning: " << arg << " is not a regular file or does not exist.\n";
            }
        }
    }
    ...
    for (const auto& path : paths) {
        ...
        if (compressed.empty()) {
            std::cout << std::left << std::setw(28) << filename 
        ...
        try {
            ...
            // Decompress (Second pass) using the best result
            Tilemap3D tm2;
            ...
            if (!outdir.empty() && identical) {
                std::vector<uint8_t> out_data(recompressed.begin(), recompressed.begin() + recompressed_size);
                fs::path out_path = fs::path(outdir) / filename;
                WriteFile(out_path.string(), out_data);
            }
